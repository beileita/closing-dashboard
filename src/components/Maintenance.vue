<template>
  <div class="flex-1 flex flex-col md:flex-row gap-4 min-h-0 px-4 md:px-6 pb-4 md:pb-6 overflow-hidden">
    <!-- 单位列表 -->
    <div class="glass rounded-xl flex flex-col w-full md:w-[400px] shrink-0 overflow-hidden">
      <div class="px-4 py-3 border-b border-tech-border flex items-center justify-between">
        <span class="text-tech-green font-medium">单位维护 <span class="text-tech-muted text-xs">({{ store.units.length }})</span></span>
      ...
      <div class="flex-1 overflow-y-auto p-2">
        <div v-for="u in store.units" :key="u.id" class="flex items-center gap-2 px-2 py-2 rounded hover:bg-tech-green/5 group">
          ...
          <span class="text-xs text-tech-muted hidden sm:inline truncate max-w-[120px]">{{ u.province }}·{{ u.city }}</span>
...
import { store, addUnit, updateUnit, deleteUnit } from '../data/store'
